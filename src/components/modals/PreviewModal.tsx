    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-[900px] max-h-[85vh] bg-ink-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <p className="text-xs text-white/60 mb-3">各平台裁切效果预览</p>
          <div className="grid grid-cols-4 gap-3">
            {platforms.map(platform => {
              const Icon = platform.icon;
              const previewScale = Math.min(platform.width / canvasSize.width, platform.height / canvasSize.height);
              const offsetX = (platform.width - canvasSize.width * previewScale) / 2;
              const offsetY = (platform.height - canvasSize.height * previewScale) / 2;
              return (
                <div key={platform.name} className="p-3 rounded-xl bg-white/5 border border-white/10">
                  <div className="flex items-center gap-1.5 mb-2">
                    <Icon size={12} className="text-neon-purple" />
